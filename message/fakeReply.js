...
const fMsg = (type, teks = defaultTeks) => {
    return {
        key: keyType[type],
        message: { 
            "extendedTextMessage": {
...
const fGif = (type, teks = defaultTeks, image = miniThumbnail) => {
    return { 
        key: keyType[type], 
        message: { 
            "videoMessage": {  
...
const fImage = (type, teks = defaultTeks, image = miniThumbnail) => { 
    return { 
        key: keyType[type],
        message: { 
            "imageMessage": { 
...
const fVideo = (type, teks = defaultTeks, image = miniThumbnail) => {
    return {
        key: keyType[type],
        message: { 
            "videoMessage": { 
...
const fDoc = (type, teks = defaultTeks) => {
    return { 
        key: keyType[type], 
        message: { 
            documentMessage: {
...
const fLoc = (type, teks = defaultTeks, image = miniThumbnail) => {
    return { 
        key: keyType[type], 
        message: { 
            "locationMessage": {
...
const fLiveLoc = (type, teks = defaultTeks) => {
    return { 
        key: keyType[type], 
        message: { 
            "liveLocationMessage": {
...
const fKontak = (type, teks = defaultTeks) => {
    return { 
        key: keyType[type], 
        message: { 
            contactMessage: {
...
const fInvite = (type = 0, teks = defaultTeks, image = miniThumbnail) => {
    return { 
...
const fSticker = (type, teks = defaultTeks) => {
    return { 
        key: keyType[type], 
        "message": {
            "stickerMessage": { 
...
const fToko = (type, teks = defaultTeks, image = miniThumbnail) => {
    return {
		key: keyType[type],
		message: {
			"productMessage": {
...
const fTroli = (type, teks = defaultTeks, image = miniThumbnail) => {
    return {
        key: keyType[type],
        message: {
            orderMessage: {
...
const fReplyCmd = ['msg', 'vn', 'image', 'video', 'toko', 'troli', 'gif', 'location', 'livelocation', 'doc', 'kontak', 'invite', 'stk']
const fakeReply = (q) => {
    const func = [fMsg, fVn, fImage, fVideo, fToko, fTroli, fGif, fLoc, fLiveLoc, fDoc, fKontak, fInvite, fSticker]
    for(let i = 0; i<fReplyCmd.length; i++){
        if(q === fReplyCmd[i].toLowerCase()){
            return func[i]
        }
    }
    return false
